...
class Tac( ServerAdmin ):
	...
	async def waiting_map(self,player,data,**kwargs):
		...
		inutile_db = mysql.connector.connect(
			host="localhost",
			user="yannex4",
			passwd="yannex123server",
			database="pyplanet_yannex4"
		)
	...
	async def delete_locals(self,player,data,**kwargs):
		loc_db = mysql.connector.connect(
			host="localhost",
			user="yannex4",
			passwd="yannex123server",
			database="pyplanet_yannex4"
		)
		cursor2 = loc_db.cursor()
		cursor2.execute("DELETE FROM `localrecord`")
	...
	async def db_debug(self,player,data,**kwargs):
		...
		message = message = '$fff{} $f{} {} {}'.format(
				
				settings.DATABASES[process]['NAME'], settings.DATABASES[process]['OPTIONS']['host'], settings.DATABASES[process]['OPTIONS']['user'], settings.DATABASES[process]['OPTIONS']['password']
			)
		await self.instance.chat(message, player)